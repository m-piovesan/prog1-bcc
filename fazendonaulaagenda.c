...
int desmarca_compromisso_agenda(agenda_t* agenda, int dia, compromisso_t* compr) {
    dia_t *listaDias = agenda->ptr_mes_atual->dias;
    compromisso_t *listaCompr = agenda->ptr_mes_atual->dias->comprs;
    ...
    /* ACHAR DIA */
    while(listaDias != NULL) {
        if(listaDias->dia == dia) {
            
            /* LISTA DE COMPROMISSOS VAZIA */
            if(listaDias->comprs == NULL)
                return 0;

            /* COMPROMISSO PRA SER REMOVIDO É O PRIMEIRO DA LISTA */
            if(listaDias->comprs == compr) {
                compromisso_t *remover = listaDias->comprs;
                listaDias->comprs = listaDias->comprs->prox;
                destroi_compromisso(remover);
             
                return 1;
            }

            /* COMPROMISSO ESTÁ NO DECORRER DA LISTA */
            while (listaCompr != NULL) {
                if(listaCompr == compr) {
                    compromisso_t *remover = listaCompr;
                    listaCompr = listaCompr->prox;
                    destroi_compromisso(remover);
                ...
                listaCompr = prox_compr(listaCompr);
            } 
        }

        listaDias = listaDias->prox;
    }
